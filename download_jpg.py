...
DUOWAN_JPG_SET_KEY = 'DUOWAN_JPG_SET_KEY'
...
def download_gif(result):
    ...
    file_name = local_path + title.replace(" ", "") + ".jpg"
    # 比较是否重复
    if redis_client.sismember(DUOWAN_JPG_SET_KEY, file_name) is True:
        # 已经存在
        print("当前文件已经下载:", file_name)
        return

    if os.path.exists(file_name):
        redis_client.sadd(DUOWAN_JPG_SET_KEY, file_name)
        print("当前文件已经下载:", file_name)
    else:
        try:
